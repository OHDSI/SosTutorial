...
We hold `Strategus` results on the OHDSI public database server `shinydb`.
...
Table \@ref(tab:schema) provides the `schema` names for some `Strategus` results.
...
tab <- kbl(schema, col.names = c("study", "schema"),
           linesep = "", booktabs = TRUE,
           caption = "Strategus study schema")
...
See, e.g., `cohortName[1, "cohortName"]` which is `r cohortName[1, "cohortName"]`.
...
makePrettyFigure(subset %>%
                   mutate(majorOrder = 1,
                          pass = unblind,
                          cumMajorSkip = 0),
                 fileName = "StrategusFigure1.png",
                 png = TRUE,
                 primaryAnalysisId = 1,
                 outcomeIds = c(19023, 19024, 19059),
                 outcomeNames = c("AMI/inpat", "AMI/all", "Diarrhea"))

knitr::include_graphics("StrategusFigure1.png")
```
